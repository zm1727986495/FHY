...
        </div>
        ...
        <!-- 地址弹框 -->
        <el-dialog title="地址管理" :visible.sync="dialogVisible" width="50%">
          <div>
        ...
        <!-- 地址弹框 -->
        <el-dialog title="地址管理" :visible.sync="orderdialogVisible" width="50%">
          <div>
        ...
        <!-- 客户信息地址 -->
        <el-dialog title :visible.sync="mapDialogVisible" width="50%" v-if="mapDialogVisible">
...
        </div>
        ...
        <div class="btn_list_1 m-bottom10" v-if="!isAudit || !isView">
        ...
        <div class="totalSalesPrice">
            <span>总售价：</span>
            <el-input v-model.number="totalPrices" :readonly = "readonly"></el-input>
            <p>元</p>
        </div>
        <el-dialog
          title="商品列表"
        ...
        >
          ...
          <div class="queryForm">
            <el-row :gutter="10" ref="queryForm">
              ...
              <el-col :span="span">
                <el-select
                  v-model="queryForm.commodityCategoryId"
                  clearable
                  placeholder="品类"
                  :size="size"
                >
              ...
              <el-col :span="span">
                ...
                <el-button type="primary" @click="resetSubmit" :size="size" plain>重置</el-button>
              </el-col>
            </el-row>
          </div>
          ...
          <div class="table_list lvs attribute">
            ...
            >
              ...
              <el-table-column prop="commodityName" label="商品名称" :width="columnWidth" :align="columnAlign" show-overflow-tooltip></el-table-column>
              <el-table-column prop="commodityModel" label="型号" :width="columnWidth" :align="columnAlign" show-overflow-tooltip></el-table-column>
              <el-table-column prop="commodityPrice" label="单价" :width="columnWidth" :align="columnAlign"></el-table-column>
              <el-table-column prop="valuationMethod" label="计价方式" :width="columnWidth" :align="columnAlign"></el-table-column>
              <el-table-column prop="classifyName" label="分类" :width="columnWidth" :align="columnAlign" show-overflow-tooltip></el-table-column>
              <el-table-column prop="constructionPeriod" label="工期" :width="columnWidth" :align="columnAlign"></el-table-column>
              <el-table-column prop="commodityUnit" label="单位" :width="columnWidth" :align="columnAlign" show-overflow-tooltip></el-table-column>
              <el-table-column prop="commodityWide" label="宽" :width="columnWidth" :align="columnAlign"></el-table-column>
              <el-table-column prop="commodityHigh" label="高" :width="columnWidth" :align="columnAlign"></el-table-column>
              <el-table-column prop="commodityLong" label="深" :width="columnWidth" :align="columnAlign"></el-table-column>
              <el-table-column prop="commodityThick" label="厚" :width="columnWidth" :align="columnAlign"></el-table-column>
              <el-table-column prop="status" label="状态" :width="columnWidth" :align="columnAlign">
              ...
              <el-table-column prop="commodityDescribe" label="备注" :width="columnWidth" :align="columnAlign" show-overflow-tooltip></el-table-column>
            </el-table>
          </div>
        ...
        <div class="table_list lvs attribute m-top20 samplecancle_table">
          <el-table
            ...
            :data="tabList"
            style="width: 100%;"
            ref="cancleSampleTable"
          >
            <el-table-column type="selection" width="55" :reserve-selection="true"></el-table-column>
            <el-table-column type="index" width="55"></el-table-column>
            <el-table-column width="240" prop="commodityCode" label="商品编码" sortable align="center"></el-table-column>
            <el-table-column width="240" prop="commodityName" label="商品名称" sortable align="center"></el-table-column>
            <el-table-column width="240" prop="brandName" label="品牌" sortable align="center"></el-table-column>
            <el-table-column width="240" prop="categoryName" label="品类" sortable align="center"></el-table-column>
      ...
      <!-- <div class="bordermodel">
		...
		<div  v-if="isAudit || isView" style="marginTop: 10px;">
	...
	<div class="bottom_btnlist" v-if="isAudit">
      <el-button type="primary" @click="auditFun('sampleForm',2)" :loading="btnLoading">确 定</el-button>
    ...
    <div class="bottom_btnlist" v-else>
      ...
      <el-button type="warning" @click="submitForm('sampleForm',1)" :loading="btnLoading">保 存</el-button>
      <el-button @click="cancle('sampleForm')">取 消</el-button>
    </div>
...
export default {
  ...
  created() {
	...
	if(this.$route.path == "/samplemanagement/viewcancle") { // 详情页面
		this.isView = true;
		this.readonly = true;
	}
  ...
  methods: {
    ...
    getDetails(){
      ...
      cancelSampleSingleDetail(data).then(res=>{
		console.log(res,"res ++++++++++++");
		this.cancelSampleSingleCode = res.data.cancelSampleSingleCode;
        // this.befoIn = res.data;
        // this.tabList = res.data.cancelSampleDetails.map(item=>{
        //   return {...item,purchaseNumber:item.cancelNumber}
		// });
		let tab = res.data.cancelSampleDetails;
		tab.forEach(item => {
			let data = {
		...
		this.totalPrices = res.data.totalPrices; // 总售价
	...
	//   if((this.$refs.cancleSampleTable.length > 0)) {                
	// 		this.$message.warning("商品");
	// 		return;
	// 	}
    ...
    //   getUUID({ serialNumber: "WS" }).then(res => {
		...
		// data.cancelSampleSingleId = this.$route.query.cancelSampleSingleId;
        // editCancelSampleSingle(data)
        //   .then(res => {
        //     console.log(res);
        //     if (res.status == 200) {
        //       if (isType == 1) {
        ...
        //   .catch(error => {
        //     console.error(
        //       "样品管理:新增撤样 views/administrationCenter/samplemanagement/cancelsample/sampleform.vue",
        //       error
        //     );
        //   });
    //   });
	...
	auditFun() {
            ...
            let data = {
				...
				cancelSampleSingleId: this.$route.query.cancelSampleSingleId,// 撤样申请id
                status: this.$refs.auditcomponent.audit.operationStatus, // 审核类型
            }

            auditCancelSampleSingle(data).then(res => {
                if(res.status == 200) {
                    this.$message.success("操作成功");
                    this.$router.push("/samplemanagement/cancelsample");
                }
            }).catch(err =>{
                console.log('样品管理:新增撤样 views/administrationCenter/samplemanagement/cancelsample/editcancle.vue', err);
            })
	},
      ...
      if (addarr.length > 1) {
        ...
        getCustomerAddress(datas)
          ...
          .catch(error => {
            console.log(
              error,
              "订单管理:根据客户姓名获取客户信息 order/justOrder/addJustOrder.vue"
            );
          });
      }
    },
    ...
    confirm() {
      ...
      baiduMap(this.center.lng, this.center.lat)
        .then(res => {
          this.mapData = res;
          let str = "";
          str +=
            this.mapData.formatted_address +
            this.mapData.sematic_description +
            "#" +
            this.buildingNumber +
            "#" +
            this.unitNumber +
          ...
          this.maps =
            this.mapData.formatted_address + this.mapData.sematic_description;

      ...
      <div class="head_nav">
		...
		<el-card class="lvs">
            ...
            <div class="order-btn-list">
                ...
                <!-- 已入库 -->
                <el-button size="small" class="button-96" @click="adminAuditFun" v-if="isAdmin || isleader">行政部审核</el-button>
            </div>
            <!-- table表格 -->
                    ...
                    <el-table-column prop="chargeBackNumber" label="退货单号" width="150" sortable align="center"  fixed='left'>
                        <template slot-scope="scope">
                            <div>
                                ...
                                <el-button type="text" @click="backidetils(scope.row)">{{scope.row.chargeBackNumber}}</el-button>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column prop="auditStatus" align="center" label="审核状态" width="120">
                        <template slot-scope="scope">
                            <div>
                                ...
                                <p class="text" v-if="scope.row.auditStatus == 3">领导审核</p>
                                <p class="text" v-if="scope.row.auditStatus == 4">驳回</p>
                            </div>       
                        </template>
                    </el-table-column>
            ...
            <!-- 点击审核记录的操作按耨--审核记录 -->
            <examinedia v-if="examVisible" @close_examin="examineclose" @examin_ok="examinok" :auditStatus='auditStatus' :chargeBackId="chargeBackId"></examinedia>
...
export default {
	...
	// 头部面包屑
        ...
        if(this.$route.path == "/regoodspermit/regoodsapply"){ // 退货下单申请人列表
            this.isApply = true;
        }else if(this.$route.path == "/regoodspermit/regoodsadmin"){ // 退货行政审核
            this.isAdmin = true;
        }else if(this.$route.path == "/regoodspermit/regoodsorder"){ // 退货下单管理
            this.isOrder = true;
        }else if(this.$route.path == "/regoodspermit/leaderexamine"){ // 退货行政领导审核
            this.isleader = true;
        }
    },
    // activated(){
    ...
    methods: {
        ...
        backidetils(val){
            this.$router.push({path: "/regoodspermit/viewapplication",query: {chargeBackId: val.chargeBackId}})
        },
        // 行政员审核
        adminAuditFun() {
            if(this.elsectVal.length > 1) {
            ...
            //     })
            // }
            if(this.elsectVal[0].auditStatus == 2){
                this.$message.warning("审核已通过，不可重复审核");
                return;
            }
            // 行政二审核
            if(this.isAdmin){
                this.$router.push({path: "/regoodspermit/adminsecaudit",query: {chargeBackId: this.elsectVal[0].chargeBackId}})
            }else if(this.isleader){
                this.$router.push({path: "/regoodspermit/adminsleadeaudit",query: {chargeBackId: this.elsectVal[0].chargeBackId}})
            }
        },
        // 获取门店
        getStore() {
        ...
        searchdatas(){
            this.loading = true;
            return {
                searchText: this.regoods.names,//订单编号
                storeId: this.regoods.storeId,//门店id
       ...
       // 获取表格数据
		getList(data){
            ...
            if(this.$route.path == "/regoodspermit/regoodsapply") { // 退货退款申请
                ...
                // 审核人列表
                this.listAuditOrderChargeBack(data);
            }else if(this.$route.path == "/regoodspermit/leaderexamine") { // 退货行政领导管理
                // 审核人列表
                this.listAuditOrderChargeBack(data);
            }

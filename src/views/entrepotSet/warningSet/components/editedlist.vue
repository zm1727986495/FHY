    ...
    <el-card>
      ...
      <div class="table_list lvs attribute">
        ...
        >
          ...
          <el-table-column align="center" prop="companyName" label="供应商" width="200"></el-table-column>
          <el-table-column
            align="center"
            prop="classifyName"
            label="商品分类"
          ...
          ></el-table-column>
          <el-table-column align="center" prop="commodityUnit" label="单位" width="200"></el-table-column>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      getStorageWarningCommodityList(data)
        .then(res => {
      ...
      getById(data)
        .then(res => {
          this.ruleForm = {
            ...
            storageId: String(res.data.storageId),
            status: String(res.data.status)
          };
        })
        .catch(error => {
          console.error("回显详情", error);
        });
    },
    //获取仓库
    getWarehouse() {
      getStorageList()
        .then(res => {
          this.warehouseList = res.data.map(item => {
            item.storageId = String(item.storageId);
            return { ...item };
          });
        })
        .catch(error => {
    ...
    //删除
    deleteFun() {
      this.choseArr.forEach((item, index) => {
        if (item.storageWarningStrategyCommodityId !== undefined) {
          this.tableData = this.tableData.filter(attr => {
            if (
              item.storageWarningStrategyCommodityId !==
              attr.storageWarningStrategyCommodityId
            ) {
              return true;
            } else {
              this.oldtableData.push({ ...attr, isDelete: 1 });
              return false;
            }
          });
        } else {
    ...
    onSubmit() {
      ...
      this.tableData.forEach(item => {
        arr.push({
          ...
          storageWarningCommodityId: item.storageWarningCommodityId,
          storageWarningStrategyCommodityId:
            item.storageWarningStrategyCommodityId
        });
      });
      arr = [...arr, ...this.oldtableData];
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          ...
          editStorageWarningStrategy(data)
            .then(res => {
              console.log(res);
              if (res.status == 200) {

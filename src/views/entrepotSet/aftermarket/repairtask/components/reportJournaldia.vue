<template>
  <div class="reportJournaldia">
    <el-dialog title="维修汇报日志" :visible.sync="visible" width="60%" :before-close="handleClose" custom-class="customDialogTitle">
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="150px" class="formclass">
        <el-form-item label="维修结果" prop="installationResult">
        ...
        <el-form-item label="维修定位">
          <el-input v-model="ruleForm.address" disabled></el-input>
        </el-form-item>

        <el-form-item label="维修前照片" required>
        ...
        <el-form-item label="维修后照片" required>
          <ul v-if="isDisabled" class="imgList">
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="addreport">提 交</el-button>
...
export default {
  ...
  data() {
    return {
      ruleForm: {},
      rules: {
        installationResult: [{ required: true, message: "请选择维修结果", trigger: "change" }],

        address: [{ required: true, message: "请输入地址", trigger: "blur" }],
        
        isRemould: [{ required: true, message: "请选择是否改制", trigger: "change" }],

        // isAbnormal: [{ required: true, message: "请选择是否异常", trigger: "change" }],
        
        isCommodityLead: [{ required: true, message: "请选择是否带回商品", trigger: "change" }],

        isRetreat: [{ required: true, message: "请选择是否可退", trigger: "change" }],

        chargeType: [{ required: true, message: "请选择是否收费", trigger: "change" }],

        unfinishedCause: [{type: "array",required: true,message: "请至少未完成原因",trigger: "change"}],

        reportContent: [{ required: true, message: "请输入地址", trigger: "blur" }]
      },
      ...
      installationReportDetails(data).then(res => {
        if (res.status == "200") {
          if (res.data == null) {
            ...
            this.ruleForm.installationResult = this.ruleForm.installationResult ? this.ruleForm.installationResult.toString() : "";
            this.ruleForm.isRemould = this.ruleForm.isRemould ? this.ruleForm.isRemould.toString() : "";
            this.ruleForm.isCommodityLead = this.ruleForm.isCommodityLead ? this.ruleForm.isCommodityLead.toString() : "";
            this.ruleForm.isAbnormal = this.ruleForm.isAbnormal ? this.ruleForm.isAbnormal.toString() : "";
            this.ruleForm.chargeType = this.ruleForm.chargeType ? this.ruleForm.chargeType.toString() : "";
            this.ruleForm.isRetreat = this.ruleForm.isRetreat ? this.ruleForm.isRetreat.toString() : "";
            this.ruleForm.unfinishedCause = this.ruleForm.unfinishedCause ? this.ruleForm.unfinishedCause.toString() : "";

    ...
    <el-card class="box-card">
      ...
      <div class="stockquery">
        <div class="head_list">
          <div class="input-vw">
            <el-input
              placeholder="物品名称/领用编号"
              v-model="bindInfo.recipientsGroup"
              size="medium"
              class="width-100"
            ></el-input>
          </div>
          ...
          <div class="input-vw">
            <el-select
              v-model="bindInfo.recipientsStatus"
              placeholder="领用状态"
              size="medium"
              class="width-100"
            >
          ...
          <div class="input-vw">
            <el-select
              v-model="bindInfo.recipientsType"
              placeholder="申请类型"
              size="medium"
              class="width-100"
            >
      ...
      <div class="m-bottom10">
        <el-button v-if="btnJurisdiction.examination_Export" size="small" class="button-96" @click="exportFn">导出</el-button>
        <el-button v-if="btnJurisdiction.examination_Approval" size="small" class="button-96" @click="applicationFun(1)">审批通过</el-button>
        <el-button v-if="btnJurisdiction.examination_Reject" size="small" class="button-96" @click="applicationFun(2)">驳回</el-button>
      </div>
        ...
        >
          ...
          <el-table-column align="center" prop="recipientsNo" label="领用编号" width="150">
              ...
              <div>
                ...
                <p>{{scope.row.recipientsNo}}</p>
              </div>
            </template>
          </el-table-column>
          <el-table-column align="center" prop="commodityName" label="名称" width="150" show-overflow-tooltip>
          </el-table-column>
          <el-table-column align="center" prop="companyName" label="申请领用物品" width="150"></el-table-column>
  ...
  methods: {
    ...
    applicationFun(type) {
      if (this.choseArr.length < 1) {
        ...
        let data = {
          recipientsId: this.choseArr[0].recipientsId
        };
        let str = "";

        if (type == 1) {
          if (this.choseArr[0].applyStatus == "已审批") {
            this.$message.error("已审批通过,无法再次审批!");
            return;
          }
          getUUID({ serialNumber: "ST" }).then(res => {
            str = "是否审批通过?";
            data.applyStatus = 1;
            data.storageId = this.choseArr[0].storageId;
          ...
          data.applyStatus = 2;
          str = "是否驳回?";
          if (this.choseArr[0].applyStatus == "仓库审批") {
            this.$message.error("当前已经驳回,无法再次驳回!");
            return;
          }
          this.btnFn(data, str);
        }

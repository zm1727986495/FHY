<template>
  <div>
    <el-dialog title="采购单" :visible.sync="isPurchase" width="50%" top="1vh" custom-class="customDialogTitle"
		:close-on-press-escape="false"  :close-on-click-modal="false" :show-close="false">
      <div class="table_list lvs attribute">
        <el-table border :row-class-name="tableRowClassName" @selection-change="chooseChange" :data="tableList" max-height="400">
          <el-table-column type="selection" width="40"></el-table-column>
          <el-table-column align="center" prop="purchaseNumber" label="采购单号" sortable></el-table-column>
          <el-table-column align="center" prop="warehouseWarrantCode" label="采购合同号"></el-table-column>
...
export default {
  props: ["orderId"],
	...
	methods: {
		...
		// 关闭弹框
		handleClose(type) {
			this.$emit("closurePurchase",type);
		},
		// 弹框确定操作按钮
		confirmFun() {
			if (this.selectArr.length < 1) {
				...
				this.isArrivalOrder =  this.selectArr[0].isArrivalOrder;//1、到货单，2、商品
				this.handleClose(1); // 关闭弹框，触发父组件的方法
				// this.$router.push({
